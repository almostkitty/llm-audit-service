...
Основной режим — та же логистическая регрессия, что в `scripts/corpus_threshold_experiment.py`:
стандартизация признаков (как в `StandardScaler` на пилотном корпусе) и сигмоида от логита.
Параметры синхронизированы с `extra/threshold_experiment.json` (пилот *n* = 24).
